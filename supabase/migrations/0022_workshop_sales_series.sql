-- ============================================================
-- 0022 — Taller como canal separado en KPIs y serie diaria.
--
-- No toca filas existentes. Solo reemplaza las dos funciones de lectura para
-- abrir Atelier / Taller / Shopify sin contar dos veces el total.
-- ============================================================
...
as $$
  ...
  select
    ...
    (coalesce(sum(r.amount) filter (where r.channel = 'atelier'), 0)
      + coalesce((select sum(m.amount) from movements m where m.channel = 'atelier'), 0))::numeric,
    (coalesce(sum(r.amount) filter (where r.channel = 'workshop'), 0)
